    ...
    return (
        ...
        <div className='pb-2 flex space-x-2'>
            <span className='flex border h-min pl-3 w-[100px] rounded-lg'>
                <Select placeholder='Trip type' style={{ width: 200, height: 30, borderRadius: 10 }} value={orders[id].tripType} onChange={(value)=>setOrder(value,'tripType')} options={store.tripList.map(item => ({ value: item, label: item }))} /></span>
            <span className='flex border h-min pl-3 w-[100px] rounded-lg'>
                <Select placeholder='Payment method' style={{ width: 200, height: 30, borderRadius: 10 }} value={orders[id].paymentMethod} onChange={(value)=>setOrder(value,'maymentMethod')} options={store.paymentList.map(item => ({ value: item, label: item }))} /></span>
        </div>
